...
class ClassLogin:
    ...
    def draw_window(self):
        ...
        self.draw_text(
            MODE[self.mode],
            self.screen.get_width() // 2 - 40 if self.mode == MODE_LOGIN else self.screen.get_width() // 2 - 85,
            20,
            color=WHITE
        )

        # Поля ввода
        self.draw_text(TITLE_INPUT[POLE_USER], START_FORM_X, START_FORM_Y + 20, color=WHITE)
        pygame.draw.rect(
            self.screen, RED if self.input_active == POLE_USER else GRAY,
            (START_FORM_X, START_FORM_Y + STEP_Y, WIDTH_FORM, HEIGHT_FORM), 2)
        self.draw_text(self.username[-19:], START_FORM_X + 10, START_FORM_Y + STEP_Y + 5, color=WHITE)

        # Ввод пароля
        self.draw_text(TITLE_INPUT[POLE_PASSWORD], START_FORM_X, START_FORM_Y + STEP_Y * 2 + 20, color=WHITE)
        pygame.draw.rect(
        ...
        self.draw_text(MODE[self.mode],
                       ...
                       self.button_start_rect.y + 5, color=WHITE)

        color_swap_button = (101, 189, 42) if self.button_swap_rect.collidepoint(mouse_pos) else (86, 161, 36)
        # Кнопка переключения
        ...
        self.draw_text(text_button,
                       ...
                       self.button_swap_rect.y + 5, color=WHITE, text_type="button")
